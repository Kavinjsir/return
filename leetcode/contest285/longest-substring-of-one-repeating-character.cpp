...
// Reference:
// https://leetcode.com/problems/longest-substring-of-one-repeating-character/discuss/1867713/C%2B%2Bor-Segment-tree
#include <string>
#include <vector>
using namespace std;
...
public:
  ...
  int update(int in, int l, int h, int j, char ch) {
    ...
    if (j >= l && j <= m)
      update(2 * in + 1, l, m, j, ch);
    else
      update(2 * in + 2, m + 1, h, j, ch);
